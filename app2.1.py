...
class CourseScheduler:
    ...
    def run_optimization(self, monthly_weight=5, champion_weight=4,
        ...
        print("Adding quarterly distribution constraints for courses with more than 4 runs")
        ...
        # Group course runs by course and language
        course_runs = {}
        for (course, delivery_type, language, i), week_var in schedule.items():
            key = (course, language)
            if key not in course_runs:
                course_runs[key] = []
            course_runs[key].append((i, week_var))
        
        # Add quarterly constraints for courses with more than 4 runs
        # Get actual run counts from course_run_data
        for (course, language), runs in course_runs.items():
            # Get the actual number of runs from course_run_data
            run_count = self.course_run_data[
                (self.course_run_data['Course Name'] == course) & 
                (self.course_run_data['Language'] == language)
            ]['Runs'].iloc[0]
            
            if run_count > 4:
                print(f"  Adding quarterly constraints for {course} ({language}) with {run_count} runs")
                
                # For each quarter, create variables to track if at least one run is scheduled in it
                for quarter, months in quarters.items():
                    # Get all weeks in this quarter
                    quarter_weeks = [week for week, month in self.week_to_month_map.items() if month in months]
                    
                    # Create variables for each run to track if it's in this quarter
                    runs_in_quarter = []
                    for run_num, week_var in runs:
