...
class CourseScheduler:
    ...
    def generate_trainer_utilization_report(self, schedule, trainer_assignments, solver):
        ...
        for _, row in self.consultant_data.iterrows():
            ...
            # Calculate minimum days status
            min_days_status = "N/A"
            if min_days > 0:
                if assigned_days >= min_days:
                    min_days_status = "✓"
                else:
                    min_days_status = f"❌ ({assigned_days}/{min_days})"

            utilization_data.append({
                "Name": name,
                "Title": title,
                "Min Days": min_days,
                "Min Days Met": min_days_status,
                "Max Days": max_days,
    ...
    def generate_excel_report(self, schedule_df, monthly_validation_df, trainer_utilization_df,
                             monthly_weight, affinity_weight, workload_violation_weight=30, spacing_violation_weight=20,
                             min_course_spacing=2, solution_strategy="BALANCED", num_workers=8, solver_time_minutes=5,
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            params_data = {
                'Parameter': [
                    ...
                    'Total Trainers',
                    'Total Champions',
                    'Total Affinity Rules'
                ],
                'Value': [
                    datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
                    str(monthly_weight),
                    str(affinity_weight),
                    str(workload_violation_weight),
                    str(spacing_violation_weight),
                    str(min_course_spacing),
                    solution_strategy,
                    str(num_workers),
                    str(solver_time_minutes),
                    str(enforce_monthly_distribution),
                    str(prioritize_all_courses),
                    len(set(self.course_run_data['Course Name'])),
                    sum(self.course_run_data['Runs']),
                    len(self.consultant_data),
                    len(self.course_champions),
                    len(self.affinity_matrix_data)
                ]
            }
            params_df = pd.DataFrame(params_data)
            params_df.to_excel(writer, sheet_name='Optimization Parameters', index=False)

            # Format the Optimization Parameters sheet
            worksheet = writer.book['Optimization Parameters']
            ...
            worksheet.column_dimensions['A'].width = 35  # Parameter names
            worksheet.column_dimensions['B'].width = 25  # Values

            # Add Affinity Violations sheet
            if schedule_df is not None:
                violations = []
                ...
                for _, row in schedule_df.iterrows():
                    ...
                    week = row['Week']
                    language = row['Language']
                    if (course, language) not in course_schedules:
                        course_schedules[(course, language)] = []
                    course_schedules[(course, language)].append({
                        'week': week,
                        'start_date': self.weekly_calendar[week-1].strftime("%Y-%m-%d")
                    })
                    ...
                    for lang1 in c1_languages:
                        for lang2 in c2_languages:
                            if (c1, lang1) in course_schedules and (c2, lang2) in course_schedules:
                                for run1 in course_schedules[(c1, lang1)]:
                                    for run2 in course_schedules[(c2, lang2)]:
                                        w1 = run1['week']
                                        w2 = run2['week']
                                        ...
                                        if actual_gap < required_gap:
                                            violations.append({
                                                'Course 1': f"{c1} ({lang1})",
                                                'Course 2': f"{c2} ({lang2})",
                                                'Required Gap': gap_weeks,
                                                'Actual Gap': actual_gap,
                                                'Course 1 Week': w1,
                                                'Course 2 Week': w2,
                                                'Course 1 Start': run1['start_date'],
                                                'Course 2 Start': run2['start_date'],
                ...
                if violations:
                    ...
                    # Format the Affinity Violations sheet
                    worksheet = writer.book['Affinity Violations']
                    header_fill = PatternFill(start_color="CCE5FF", end_color="CCE5FF", fill_type="solid")
            ...
            with col2:
                ...
                with st.expander("View format requirements", expanded=False):
                    ...
                    2. **TrainerData**: Trainer information
                       - Columns: Name, Title, Min_Days, Max_Days
                       - Min_Days: Minimum required working days (0 for no minimum)
                       - Max_Days: Maximum allowed working days
